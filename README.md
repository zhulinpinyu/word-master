...
内置**沪教版（三起）三年级上册**与**沪教版（深圳）一年级上册**全套单词**真人录音 + 教材插图**，播放时优先命中语音包，与课本发音一致；单词本列表和测验卡片会展示配套插图。数据由 `scripts/fetch-voice-package.mjs` 生成，详见[开发文档](docs/development.md#语音包真人录音)。
